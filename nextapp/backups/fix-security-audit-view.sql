...
-- ============================================================================
-- This SQL should be run in Supabase SQL Editor to fix the last security issue
-- 
-- Issue: View `public.security_audit_status` is defined with SECURITY DEFINER
-- Solution: Recreate the view with security_invoker = true
-- ============================================================================
...
SELECT 
  viewname,
  CASE 
    WHEN viewowner IS NOT NULL THEN 'View exists'
    ELSE 'View not found'
  END as status
FROM pg_views 
WHERE schemaname = 'public' 
  AND viewname = 'security_audit_status';
