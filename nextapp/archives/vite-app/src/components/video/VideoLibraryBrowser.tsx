...
const VideoLibraryBrowser: React.FC<VideoLibraryBrowserProps> = ({
  ...
  const loadVideos = async () => {
    ...
    try {
      ...
      if (filterByType) {
        query = query.eq('video_type', filterByType);
      }

      if (topicFilter && filterByTopic) {
        query = query.contains('topic_ids', [filterByTopic]);
      }
      ...
      const resolvedVideos = (data || []).map((video: any) => {
        let resolvedUrl = video.video_url;

        if (!resolvedUrl && video.video_files) {
          const videoFile = Array.isArray(video.video_files) ? video.video_files[0] : video.video_files;
          if (videoFile?.storage_path) {
            resolvedUrl = getVideoFilePublicUrl(
              videoFile.storage_path,
              videoFile.storage_bucket || 'video-files'
            );
            console.log('[VideoLibraryBrowser] Resolved URL from video_file:', resolvedUrl);
          }
        }

        return {
          ...video,
          video_url: resolvedUrl || video.video_url
        };
      });
