...
import { BravinAssessmentResult, BravinDimensionCode } from '../../types';
...
const BravinResults: React.FC<BravinResultsProps> = ({
  ...
  const loadResults = async () => {
    setLoading(true);
    const data = await BravinMetricsService.getAssessmentResult(learnerId, simulationInstanceId);
    setResult(data);
    setLoading(false);
  };
    ...
    return (
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-8 text-center">
        ...
        <div className="max-w-2xl mx-auto text-left space-y-3">
          ...
          <p className="text-blue-800">
            This simulation may not have BRAVIN assessments enabled. This could mean:
          </p>
          <ul className="list-disc list-inside text-blue-700 space-y-1 ml-4">
            <li>The simulation scenarios haven't been configured with BRAVIN impact values</li>
            <li>No BRAVIN-enabled decisions have been recorded yet</li>
            <li>The administrator needs to set up BRAVIN mappings for this simulation</li>
          </ul>
          <div className="mt-4 pt-4 border-t border-blue-300">
            <p className="text-sm text-blue-700 italic">
              If you believe you should see BRAVIN results here, please contact your administrator
              to configure BRAVIN assessments for this simulation.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold mb-2">JMMB Leadership Culture Assessment</h2>
      ...
      {showDetailedBreakdown && (
        <>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <MetricCard
                title="Trust Impact Rating"
                score={result.trust_impact_rating}
                description="Your effect on team trust and psychological safety"
                icon={Heart}
                color="emerald"
              />
              ...
              <MetricCard
                ...
                score={result.ethical_decision_quality}
                description="Balance of performance with values under pressure"
                icon={Shield}
                color="blue"
              />

              <MetricCard
                title="Emotional Intelligence Index"
                score={result.emotional_intelligence_index}
                description="Recognition of emotions and empathetic response"
                icon={Users}
                color="purple"
              />

              <MetricCard
                title="Cultural Stewardship Score"
                score={result.cultural_stewardship_score}
                description="Active protection and shaping of organizational culture"
                icon={Award}
                color="amber"
              />
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-6">Trust Building Summary</h3>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <TrustEventCard
                label="Trust Built"
                count={result.trust_events_summary.trust_built}
                color="green"
                icon={TrendingUp}
              />
              <TrustEventCard
                label="Trust Maintained"
                count={result.trust_events_summary.trust_maintained}
                color="blue"
                icon={Minus}
              />
              <TrustEventCard
                label="Trust Repaired"
                count={result.trust_events_summary.trust_repaired}
                color="amber"
                icon={CheckCircle}
              />
              <TrustEventCard
                label="Trust Damaged"
                count={result.trust_events_summary.trust_damaged}
                color="red"
                icon={TrendingDown}
              />
            </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-blue-800 text-sm leading-relaxed">
                  The BRAVIN framework measures how consistently your actions reflect JMMB's leadership
                  ...
                  areas for development as a JMMB leader.
                </p>
              </div>
            </div>
          </div>
...
const MetricCard: React.FC<MetricCardProps> = ({ title, score, description, icon: Icon, color }) => {
  const colorClasses = {
    emerald: {
  ...
  return (
    <div className={`${classes.bg} border ${classes.border} rounded-lg p-5`}>
      <div className="flex items-center justify-between mb-3">
        <Icon className={`w-6 h-6 ${classes.text}`} />
        <span className={`text-2xl font-bold ${classes.text}`}>{Math.round(score)}</span>
      </div>
      <h4 className={`font-semibold ${classes.text} mb-2`}>{title}</h4>
      <p className={`text-sm ${classes.text} opacity-75 mb-3`}>{description}</p>
      <div className="w-full bg-white dark:bg-gray-700 rounded-full h-2 overflow-hidden">
        <div
...
const TrustEventCard: React.FC<TrustEventCardProps> = ({ label, count, color, icon: Icon }) => {
  const colorClasses = {
  ...
  return (
    <div className={`${classes.bg} border ${classes.border} rounded-lg p-4 text-center`}>
      <Icon className={`w-8 h-8 ${classes.text} mx-auto mb-2`} />
      <p className={`text-3xl font-bold ${classes.text} mb-1`}>{count}</p>
      <p className={`text-sm ${classes.text} opacity-75`}>{label}</p>
    </div>
  );
};
