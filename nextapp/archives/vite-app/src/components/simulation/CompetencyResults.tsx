...
import { Award, TrendingUp, Target, BookOpen, ExternalLink } from 'lucide-react';
import { CompetencyService, LearnerCompetency, Competency } from '../../lib/competencies';
...
const CompetencyResults: React.FC<CompetencyResultsProps> = ({ learnerId, simulationId }) => {
  ...
  const loadCompetencies = async () => {
    ...
    try {
      ...
      console.log('[CompetencyResults] Loaded learner competencies:', learnerData);
      console.log('[CompetencyResults] All competencies:', allData);
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500">{comp.description}</p>
                  </div>
                </div>
                ...
                {comp.tags && comp.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {comp.tags.slice(0, 3).map((tag, idx) => (
                      <span
                        key={idx}
                        className="text-xs px-2 py-0.5 bg-gray-200 text-gray-700 dark:text-gray-300 rounded"
                      >
                        {tag}
                      </span>
                    ))}
