...
const SimulationListView: React.FC<SimulationListViewProps> = ({
  ...
  const loadSimulations = async () => {
    setLoading(true);
    const data = await SimulationService.getSimulationsByCategory(categoryId);
    setSimulations(data);
    setLoading(false);
  };

  const handleDuplicate = async (simulation: Simulation) => {
    const newName = `${simulation.name}_copy_${Date.now()}`;
    const userId = simulation.created_by || '';

    const duplicated = await SimulationService.duplicateSimulation(
      simulation.id,
      newName,
      userId
    );
  ...
  const handleStatusChange = async (simulationId: string, status: 'draft' | 'review' | 'published' | 'archived') => {
    const success = await SimulationService.updateSimulationStatus(simulationId, status);
    if (success) {
      loadSimulations();
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      draft: { bg: 'bg-gray-100', text: 'text-gray-800 dark:text-gray-100', icon: XCircle },
      review: { bg: 'bg-amber-100', text: 'text-amber-800', icon: Clock },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <button
            onClick={onBack}
            className="flex items-center text-gray-600 hover:text-gray-900 dark:text-gray-100 transition-colors"
          >
