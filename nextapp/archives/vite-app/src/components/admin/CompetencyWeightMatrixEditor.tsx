...
export default function CompetencyWeightMatrixEditor({
  ...
  const loadWeights = async () => {
    console.log('[CompetencyWeightMatrixEditor] loadWeights called with:', { scenarioId, simulationId });
    setIsLoading(true);
    try {
      const global = await CompetencyCalculationService.getGlobalWeights();
      console.log('[CompetencyWeightMatrixEditor] Loaded global weights:', global);
      setGlobalWeights(global);

      let simWeights = {};
      if (simulationId) {
        simWeights = await CompetencyCalculationService.getSimulationWeights(simulationId);
        console.log('[CompetencyWeightMatrixEditor] Loaded simulation weights:', simWeights);
        setSimulationWeights(simWeights);
      }

      if (scenarioId) {
        const scenarioWeights = await CompetencyCalculationService.getScenarioWeights(scenarioId);
        console.log('[CompetencyWeightMatrixEditor] Loaded scenario weights:', scenarioWeights);
        const finalWeights = Object.keys(scenarioWeights).length > 0 ? scenarioWeights : (Object.keys(simWeights).length > 0 ? simWeights : global);
        console.log('[CompetencyWeightMatrixEditor] Setting final weights (scenario mode):', finalWeights);
        setWeights(finalWeights);
      } else if (simulationId) {
        const finalWeights = Object.keys(simWeights).length > 0 ? simWeights : global;
        console.log('[CompetencyWeightMatrixEditor] Setting final weights (simulation mode):', finalWeights);
        setWeights(finalWeights);
      } else {
        console.log('[CompetencyWeightMatrixEditor] Setting final weights (global mode):', global);
        setWeights(global);
      }
    } catch (error) {
  ...
  const handleWeightChange = (competencyCode: string, metricType: string, value: string) => {
    console.log('[CompetencyWeightMatrixEditor] handleWeightChange called:', { competencyCode, metricType, value });

    const numValue = parseFloat(value);
    console.log('[CompetencyWeightMatrixEditor] Parsed value:', numValue);

    if (isNaN(numValue) || numValue < 0 || numValue > 1) {
      console.warn('[CompetencyWeightMatrixEditor] Invalid value, ignoring:', { numValue, isNaN: isNaN(numValue), tooLow: numValue < 0, tooHigh: numValue > 1 });
      return;
    }
    ...
    console.log('[CompetencyWeightMatrixEditor] New weights state:', newWeights);
    ...
    if (autoSaveTimer) {
      console.log('[CompetencyWeightMatrixEditor] Clearing existing auto-save timer');
      clearTimeout(autoSaveTimer);
    }

    if (simulationId && !readOnly) {
      console.log('[CompetencyWeightMatrixEditor] Setting up auto-save timer (2 seconds)');
      const timer = setTimeout(() => {
        console.log('[CompetencyWeightMatrixEditor] Auto-save timer triggered!');
        handleSave(newWeights);
      }, 2000);
      setAutoSaveTimer(timer);
    } else {
      console.log('[CompetencyWeightMatrixEditor] Auto-save NOT set up:', { simulationId, readOnly });
    }
  };
  ...
  const handleSave = async (weightsToSave?: Record<string, Record<string, number>>) => {
    ...
    console.log('[CompetencyWeightMatrixEditor] Starting save...');
    console.log('[CompetencyWeightMatrixEditor] Simulation ID:', simulationId);
    console.log('[CompetencyWeightMatrixEditor] Current User:', currentUser);
    console.log('[CompetencyWeightMatrixEditor] Weights to save:', currentWeights);

    if (!simulationId) {
      setMessage({ type: 'error', text: 'Cannot save: Missing simulation ID' });
      console.error('[CompetencyWeightMatrixEditor] No simulation ID provided');
      return;
    }

    if (!currentUser?.id) {
      setMessage({ type: 'error', text: 'Cannot save: User not authenticated' });
      console.error('[CompetencyWeightMatrixEditor] No current user');
      return;
    }
    ...
    try {
      ...
      for (const competencyCode of Object.keys(currentWeights)) {
        ...
        console.log(`[CompetencyWeightMatrixEditor] Saving ${competencyCode}:`, competencyWeights);

        const success = await CompetencyCalculationService.setSimulationWeights(
          simulationId,
          competencyCode,
          competencyWeights,
          currentUser.id
        ...
        console.log(`[CompetencyWeightMatrixEditor] Save result for ${competencyCode}:`, success);

        if (!success) {
          console.error(`[CompetencyWeightMatrixEditor] Failed to save ${competencyCode}`);
          allSuccess = false;
          break;
        }
        saveCount++;
      ...
      console.log(`[CompetencyWeightMatrixEditor] Saved ${saveCount} competencies, allSuccess:`, allSuccess);
      ...
      setMessage({ type: 'error', text: `Error saving weights: ${error instanceof Error ? error.message : 'Unknown error'}` });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const calculateRowSum = (competencyCode: string): number => {
    ...
    return METRIC_TYPES.reduce((sum, metric) => {
      return sum + (competencyWeights[metric.key] || 0);
    }, 0);
  };
