...
export class ErrorHandler {
  ...
  private static createAppError(
    ...
    if (error instanceof Error) {
      ...
      message = error.message;
      code = error.code;
      severity = this.getPostgrestErrorSeverity(error);
      recoverable = this.isPostgrestErrorRecoverable(error);
      userMessage = userMessage || this.getPostgrestUserMessage(error);
    }
  ...
  private static getPostgrestErrorSeverity(error: PostgrestError): ErrorSeverity {
  ...
  private static isPostgrestErrorRecoverable(error: PostgrestError): boolean {
    const unrecoverableCodes = ['PGRST301', '42501', '42P01'];
    return !unrecoverableCodes.includes(error.code || '');
  }

  private static getPostgrestUserMessage(error: PostgrestError): string {
