...
-- Note: Storage bucket creation is done through Supabase Storage API
-- This migration documents the configuration that should be applied
...
VALUES (
  ...
  524288000, -- 500MB file size limit (500 * 1024 * 1024)
  ARRAY[
    'video/mp4',
    'video/webm',
...
-- ============================================================================
-- STORAGE POLICIES FOR VIDEO FILES BUCKET
-- ============================================================================

-- Policy: Allow authenticated instructors and admins to upload files
CREATE POLICY IF NOT EXISTS "Instructors and admins can upload video files"
...
-- Policy: Allow authenticated users to read/view all video files
CREATE POLICY IF NOT EXISTS "Authenticated users can view video files"
ON storage.objects FOR SELECT
TO authenticated
USING (bucket_id = 'video-files');

-- Policy: Allow public access to read video files (for learners)
CREATE POLICY IF NOT EXISTS "Public can view video files"
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'video-files');

-- Policy: Allow file uploaders and admins to update their files
CREATE POLICY IF NOT EXISTS "Uploaders and admins can update video files"
...
-- Policy: Allow file uploaders and admins to delete their files
CREATE POLICY IF NOT EXISTS "Uploaders and admins can delete video files"
