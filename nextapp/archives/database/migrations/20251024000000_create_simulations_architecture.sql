/*
  ...
  1. New Tables
    - `simulations` - Core learning paths composed of scenarios
      - `id` (uuid, primary key)
      - `name` (text, unique) - Unique simulation identifier
      - `display_name` (text) - User-friendly display name
      - `description` (text)
      - `category_id` (uuid) - Link to simulation_categories
    ...
    - `simulation_scenarios` - Junction table linking scenarios to simulations
      - `id` (uuid, primary key)
      - `simulation_id` (uuid) - Link to simulations
      - `scenario_id` (uuid) - Link to scenarios
      - `is_entry_point` (boolean) - First scenario in flow
      - `is_exit_point` (boolean) - Last scenario in flow
      - `sequence_order` (integer) - Optional ordering
      - Timestamps

  2. Modified Tables
    - `scenarios` - Remove category_id as scenarios are now building blocks
...
-- Add scenario_id to category_id constraint removal (scenarios are now building blocks)
-- We'll keep the category_id field for now for backwards compatibility but it's deprecated
...
-- Update training_assignments to reference simulation_id instead of topic/scenario list
