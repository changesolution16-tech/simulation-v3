...
import type { Competency, ProficiencyLevel } from '@/lib/competencies';
...
const CompetencyManager: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingCompetency) {
        const response = await fetch(`/api/competencies/${editingCompetency.id}`, {
          method: 'PATCH',
      ...
      alert(error.message || 'Failed to save competency');
    }
  };
  ...
  const updateProficiencyLevel = (index: number, field: keyof ProficiencyLevel, value: string | number) => {
    ...
    newLevels[index] = { ...newLevels[index], [field]: value } as ProficiencyLevel;
  ...
  const renderCompetencyTree = (parentId?: string | null, level = 0) => {
    const children = competencies.filter((competency) =>
      parentId === undefined || parentId === null
        ? competency.parent_competency_id === null || competency.parent_competency_id === undefined
        ...
        <div className="grid grid-cols-4 gap-4 text-sm">
          ...
          <div className="bg-white dark:bg-gray-800 rounded p-3">
            ...
            <div className="text-2xl font-bold text-green-600">
              {competencies.filter(c => c.is_active !== false).length}
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded p-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, competency_level: parseInt(e.target.value, 10) })}
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      min="1"
                    />
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {(formData.proficiency_levels || []).map((level, index) => (
                      <div key={index} className="flex gap-3 items-start p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                        <div className="w-16">
                          <input
                            type="number"
                            value={level.level}
                            onChange={(e) => updateProficiencyLevel(index, 'level', parseInt(e.target.value, 10))}
